...
int main(int argc, char **argv)
{
    ...
    int *repartoFilas = (int *)malloc(sizeof(int) * nThreads);
    ...
    for(int i = 1; i < n; i++)
    {
        if(reparto[i] == reparto[i-1])
            rankAssignement++;
        
        else
            rankAssignement = 0;

        repartoFilas[i] = rankAssignement;
    }
    ...
    // int *sendCounts array donde se almacena el numero de elementos que se envia a cada vector


    // TODO:
    // Preparar sendCOunts y displs correctamente para el scatterv
    ...
    MPI_Scatterv(&caminos[0][0], sendCounts, &displs[0], MPI_INT, &auxCaminos[0][0], n, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Scatterv(&dist[0][0], sendCounts, &displs[0], MPI_FLOAT, &auxPesos[0][0], n, MPI_FLOAT, 0, MPI_COMM_WORLD);

    printf("Scatterv bien %d\n", rank);
    ...
    for(int k = 0; k < n; k++)
    {
        // De lo que tiene cada uno, reparto nos dice que hilo.
        // Necesitamos saber que fila de cada hilo
        //MPI_Bcast(auxCaminos, n, MPI_INT, reparto[k], MPI_COMM_WORLD);

        // k es la fila, reparto[k] nos devuelve que proceso tiene esa fila

        MPI_Bcast(&auxCaminos[repartoFilas[k]][0], n, MPI_INT, reparto[k], MPI_COMM_WORLD);
        MPI_Bcast(&auxPesos[repartoFilas[k]][0], n, MPI_FLOAT, reparto[k], MPI_COMM_WORLD);
